...
if (file_exists($requestedFile) && is_file($requestedFile)) {
    ...
    if (pathinfo($requestedFile, PATHINFO_EXTENSION) === 'php' && dirname($requestedFile) === $documentRoot) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && preg_match('/^\/backend\/.*\.php$/', $uri)) {
...
http_response_code(404);
echo '<h1>404 Not Found</h1>';
echo '<p>The requested resource <code>' . htmlspecialchars($uri) . '</code> was not found on this server.</p>';
